...
function sort(countries: CompaniesModel[], column: SortColumn, direction: string): CompaniesModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: CompaniesModel, term: string, pipe: PipeTransform) {
  return country.img.toLowerCase().includes(term.toLowerCase())
  || country.companyName.toLowerCase().includes(term.toLowerCase())
  || country.owner.toLowerCase().includes(term.toLowerCase())
  || country.industryType.toLowerCase().includes(term.toLowerCase())
...
export class CompaniesService {
  ...
  private _countries$ = new BehaviorSubject<CompaniesModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let countries = sort(Companies, sortColumn, sortDirection);
