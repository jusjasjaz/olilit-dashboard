...
function sort(countries: ContactsModel[], column: SortColumn, direction: string): ContactsModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: ContactsModel, term: string, pipe: PipeTransform) {
  ...
  || country.score.toLowerCase().includes(term.toLowerCase())
...
export class ContactsService {
  ...
  private _countries$ = new BehaviorSubject<ContactsModel[]>([]);
